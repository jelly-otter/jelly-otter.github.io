document.addEventListener("DOMContentLoaded", () => {
    ...
    const loadTimelineData = async () => {
        try {
            ...
            // 获取排序设置
            const settings = JSON.parse(localStorage.getItem("settings") || "{}");
            const isReverseOrder = settings.timelineOrder !== undefined ? settings.timelineOrder : true;
    ...
    const loadSettings = () => {
        const settings = JSON.parse(localStorage.getItem("settings") || "{}");
        // 设置默认值为正序（true 表示正序）
        timelineOrder.checked = settings.timelineOrder !== undefined ? settings.timelineOrder : true;

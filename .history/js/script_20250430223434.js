        ...
        key.addEventListener("click", () => {
            ...
            if (value === "delete") {
                if (enteredPassword.length > 0) {
                    // 添加删除动画
                    dots[enteredPassword.length - 1].classList.add("deleting");
                    setTimeout(() => {
                        enteredPassword = enteredPassword.slice(0, -1);
                        dots[enteredPassword.length].classList.remove("filled");
                        dots[enteredPassword.length].classList.remove("deleting");
                    }, 150);
                }
...
function createTimelineItem(eventData) {
    ...
    try {
        ...
        if (!isNaN(eventDate)) {
            eventDateStr = eventDate.toLocaleDateString("zh-CN", dateOptions);
            // 计算距离今天的天数
            const today = new Date();
            today.setHours(0, 0, 0, 0);
    ...
    if (eventData.interaction) {
        ...
        if (eventData.interaction.type === "memory" && eventData.interaction.feedback) {
            ...
            contentHTML += `<button class="interaction-button" onclick="showFeedback('${feedbackId}')">回忆一下 <span class="days-ago">(${daysDiff}天前)</span></button>`;
            contentHTML += `<p id="${feedbackId}" style="display: none;">${eventData.interaction.feedback.default || ''}</p>`;
